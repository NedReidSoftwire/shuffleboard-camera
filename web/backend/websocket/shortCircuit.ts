...
const calculateDistance = (discs: DiscWithIndex[]) => {
  return discs.reduce((smallestDist: Distance | undefined, currentDisc) => {
    const minDistFromCurrentDisc = discs.reduce(
      (smallestDist: Distance | undefined, discToCompare) => {
    ...
    if (
      minDistFromCurrentDisc &&
      (!smallestDist || minDistFromCurrentDisc.distance < smallestDist.distance)
    ) {
...
const calculateWinner = (
  blueDistance: Distance | undefined,
  redDistance: Distance | undefined,
) => {
